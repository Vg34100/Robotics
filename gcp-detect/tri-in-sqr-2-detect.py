...
def detect_x_pattern_squares():
    ...
    # Parameters for detection - exposing them at the top for easy tuning
    ...
    while True:
        ...
        # Display the detected X-pattern squares on the main image
        for x, y, w, h, approx, confidence in x_pattern_squares:
            # Green box around the detected pattern
            cv2.rectangle(display_frame, (x, y), (x + w, y + h), (0, 255, 0), 3)

            # Add confidence score
            cv2.putText(display_frame, f"X Pattern ({confidence:.1f})", (x, y-10),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Display results
        cv2.imshow('X Pattern Detection', display_frame)

        # Save the final output every N frames
        if frame_count % debug_interval == 0:
            save_debug_image(f"11_final_output_{frame_count}", display_frame)
